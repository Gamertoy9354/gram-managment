...
export async function sendBulkMessage(
  ...
  return apiFetch("/api/broadcast/send", {
    method: "POST",
    body: JSON.stringify({ message, recipients, imageUrl, templateName, templateValues }),
  });
}
...
export async function createTemplate(name: string, title: string, body: string, type = "broadcast", formId?: string) {
  return apiFetch("/api/templates", {
    method: "POST",
    body: JSON.stringify({ name, title, body, type, formId }),
  });
}

export async function deleteTemplate(name: string) {
  return apiFetch(`/api/templates/${name}`, {
    method: "DELETE",
  });
}
...
export async function updateConfigs(configs: Record<string, string>) {
  return apiFetch("/api/config", {
    method: "PUT",
