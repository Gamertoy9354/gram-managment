...
import { fetchForms, createForm, deleteForm, fetchTemplates } from "@/lib/api";
...
export default function BlankFormsPage() {
  ...
  // Load configured blank forms and templates
  const loadFormsList = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load forms/templates:", err);
  ...
  // Poll templates status if any is pending
  useEffect(() => {
    const hasPending = templates.some(t => t.status === "pending" && t.type === "form");
    if (hasPending) {
      const interval = setInterval(async () => {
        try {
          ...
          console.error("Polling templates failed:", err);
        }
      }, 3000);
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", flex: 1 }}>
                      <div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                          ...
                          {(() => {
                            const linkedTemplate = templates.find(t => t.formId === form.id);
                            ...
                            return (
                              <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.72rem" }}>
                                ...
                                <span style={{ fontFamily: "monospace", color: "var(--text-secondary)", fontWeight: 600 }}>{linkedTemplate.name}</span>
                                {linkedTemplate.status === "pending" ? (
                                  <span className="badge badge-warning" style={{ fontSize: "0.65rem", padding: "1px 6px", textTransform: "none" }}>Pending Meta Review</span>
                                ) : (
                                  <span className="badge badge-success" style={{ fontSize: "0.65rem", padding: "1px 6px", textTransform: "none" }}>Verified</span>
                                )}
                              </div>
                            );
                          })()}
