...
async function main() {
  console.log('0. Preparing database by cleaning up any existing test mobile numbers...');
  const { supabaseAdmin } = require('../src/config/supabase');
  await supabaseAdmin.from('tax_records').delete().in('mobile_number', ['9537199300', '9824567406']);
  await supabaseAdmin.from('tax_records').delete().in('property_id', ['TX101', 'TX102']);
  ...
  if (!lastMessageText.includes('UPI Payment Link and QR Code Generated')) {
    console.error('❌ Failed to generate secure UPI link!');
    process.exit(1);
  }
  console.log('✓ Bot successfully generated secure UPI link!');

  console.log('\n6. Simulating Mock Trustope Webhook payment capture...');
  ...
  if (updatedRecord.receipt_pdf_url) {
    ...
    console.log('\n8. [Skipping] No online PDF receipt generated (physical collection required).');
  }
