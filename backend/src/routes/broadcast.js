...
const { client, sendMessage, sendMedia, sendTemplateMessage } = require('../config/twilio');
...
router.post('/send', async (req, res) => {
  const { message, recipients, imageUrl, templateName, templateValues } = req.body;

  if (!message && !templateName) {
    return res.status(400).json({ error: 'Message content or WhatsApp Template is required.' });
  }
  if (!recipients) {
  ...
  try {
    ...
    for (const citizen of citizens) {
      ...
      try {
        let response;
        if (templateName) {
          // Resolve variable values replacing placeholder with citizen's name
          const resolvedValues = (templateValues || []).map(val => {
            if (typeof val === 'string') {
              let resVal = val.replace(/{name}/gi, citizen.full_name);
              return resVal.replace(/{{name}}/gi, citizen.full_name);
            }
            return val;
          });
          response = await sendTemplateMessage(formattedTo, templateName, resolvedValues, imageUrl);
        } else {
        ...
        if (deliveryStatus === 'success') {
          ...
          await logTransaction({
            ...
            documentRequested: `[Broadcast] ${(templateName || message || '').slice(0, 40)}...`,
            status: 'success',
            sessionId: null,
          });
